...
class DDQNSingle(DDQN):
    def __init__(self, CONFIG, numAction, actionList, dimList,
                    mode='normal', actType='Tanh', skip=False):
        ...
        self.build_network(dimList, actType, skip=skip)


    def build_network(self, dimList=None, actType='Tanh', skip=False):
        self.Q_network = model(dimList, actType, skip=skip, verbose=True)
        self.target_network = model(dimList, actType, skip=skip)
    ...
    def update(self, addBias=False):
        ...
        if self.mode == 'RA':
            ...
            if addBias: # Bias version: V(s) = gamma ( max{ g(s), min{ l(s), V_diff(s') } } - max{ g(s), l(s) } ),
                ...
                non_terminal = V_better  # torch.max(min_term, g_x)
                terminal = torch.max(l_x, g_x)
    ...
    def initQ(self, env, warmupIter, num_warmup_samples=200, vmin=-1, vmax=1):
        ...
        for ep_tmp in range(warmupIter):
            ...
            nn.utils.clip_grad_norm_(self.Q_network.parameters(), self.max_grad_norm)
            self.optimizer.step()
            print('\rWarmup Q [{:d}], loss={:f}, lr={:f}'.format(
                ep_tmp+1, loss.item(),
                self.optimizer.param_groups[0]['lr']), end='')

        print(" --- Warmup Q Ends")
        # env.visualize(self.Q_network, nx=41, ny=41, vmin=vmin, vmax=vmax, cmap='seismic')
        # plt.pause(0.001)
    ...
    def learn(  self, env, MAX_UPDATES=2000000, MAX_EP_STEPS=100,
        ...
        # == Warmup Q ==
        if warmupQ:
            self.initQ(env, warmupIter=warmupIter)
