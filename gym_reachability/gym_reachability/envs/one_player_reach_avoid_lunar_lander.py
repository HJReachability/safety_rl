...
class OnePlayerReachAvoidLunarLander(MultiPlayerLunarLanderReachability):
        ...
        for y_jj, y_dot in enumerate(self.slices_y):
            for x_ii, x_dot in enumerate(self.slices_x):
                ...
                v, xs, ys = self.get_value(
                    q_func, nx, ny, x_dot=x_dot, y_dot=y_dot, theta=0,
                ...
                if boolPlot:
                    if trueRAZero:
                        ...
                        xs = np.linspace(
                            self.bounds_simulation[0, 0],
                            self.bounds_simulation[0, 1], nx1
                        )
                        ys = np.linspace(
                            self.bounds_simulation[1, 0],
                        ...
                        while not it.finished:
                            ...
                            it.iternext()
                        ax.imshow(
                            resultMtx.T != 1, interpolation='none',
                            extent=axStyle[0], origin="lower", cmap=cmap
                        )
                    else:
                        ax.imshow(
                            v.T > lvlset, interpolation='none',
                            extent=axStyle[0], origin="lower", cmap=cmap
                        )
                    X, Y = np.meshgrid(xs, ys)
                    ax.contour(
                        X, Y, v.T, levels=[-0.1], colors=('k',),
                    ...
                    ax.imshow(
                        ...
                        origin="lower", cmap=cmap, vmin=-vstar, vmax=vstar
                    )
                    X, Y = np.meshgrid(xs, ys)
                    ax.contour(
                        X, Y, v.T, levels=[-0.1], colors=('k',),
    ...
    def get_warmup_examples(self, num_warmup_samples=100, s_margin=True):
        ...
        rv = np.random.uniform(
            low=self.bounds_simulation[:, 0], high=self.bounds_simulation[:,
                                                                          1],
