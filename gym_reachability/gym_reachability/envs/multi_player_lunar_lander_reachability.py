...
class MultiPlayerLunarLanderReachability(gym.Env, EzPickle):
    ...
    def __init__(
        ...
        self.chunk_x = [
            self.W / (self.CHUNKS - 1) * i for i in range(self.CHUNKS)
        ]
        self.chunk_y = [
            self.H / (self.CHUNKS - 1) * i for i in range(self.CHUNKS)
        ]
    ...
    def _generate_param_dict(self, input_dict):
        ...
        param_dict["LANDER_POLY"] = [(-14, +17), (-17, 0), (-17, -10),
                                     (+17, -10), (+17, 0), (+14, +17)]
    ...
    def generate_lander(self, initial_state, key):
        ...
        for i in [-1, +1]:
            leg = self.world.CreateDynamicBody(
                position=(
                    initial_x - i * self.LEG_AWAY / self.SCALE, initial_y
    ...
    def reset(self, state_in=None, terrain_polyline=None):
        ...
        if (
            self.doneType == 'toThreshold'
            or self.doneType == 'toFailureOrSuccess'
        ):
            self.world.contactListener = None
            self._clean_particles(True)
            self.world.DestroyBody(self.moon)
    ...
    def parent_step(self, action, key):
        ...
        # Engines
        tip = (
            math.sin(self.lander[key].angle), math.cos(self.lander[key].angle)
        )
        side = (-tip[1], tip[0])
        ...
        if self.observation_type == "LiDAR":
            ...
            for ii in range(num_beams):
                ...
                self.lidar[key][ii].p2 = (
                    pos[0] +
                    math.sin(2.0 * np.pi * ii / num_beams) * self.LIDAR_RANGE,
                    pos[1]
                    + math.cos(2.0 * np.pi * ii / num_beams) * self.LIDAR_RANGE
                )
                self.world.RayCast(
                    self.lidar[key][ii], self.lidar[key][ii].p1,
                    self.lidar[key][ii].p2
                )
        ...
        if action == 2:
            ...
            ox = tip[
                0
            ] * 4 / self.SCALE  # 4 is move a bit downwards, +-2 for randomness
            oy = -tip[1] * 4 / self.SCALE
            impulse_pos = (
                self.lander[key].position[0] + ox,
    ...
    def render(self, mode='human', plot_landers=True, **kwargs):
        ...
        if self.viewer is None:
            ...
            self.viewer.set_bounds(
                0, self.VIEWPORT_W / self.SCALE, 0,
        ...
        for obj in self.particles:
            obj.ttl -= 0.15
            obj.color1 = (
                max(0.2, 0.2 + obj.ttl), max(0.2, 0.5 * obj.ttl),
                max(0.2, 0.5 * obj.ttl)
            )
            obj.color2 = (
                max(0.2, 0.2 + obj.ttl), max(0.2, 0.5 * obj.ttl),
                ...
                for f in obj.fixtures:
                    ...
                    if type(f.shape) is circleShape:
                        t = rendering.Transform(
                            translation=trans * f.shape.pos
                        )
                        self.viewer.draw_circle(
                            f.shape.radius, 20, color=obj.color1
                        ).add_attr(t)
                        self.viewer.draw_circle(
                            f.shape.radius, 20, color=obj.color2, filled=False,
                            linewidth=2
                        ).add_attr(t)
                    else:
