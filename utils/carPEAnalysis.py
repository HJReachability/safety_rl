...
def plotTrajStep(
    ...
    for i in range(trajEvader.shape[0]):
        ...
        env.plot_target_failure_set(
            ax=ax, xPursuer=xPursuer, yPursuer=yPursuer
        )
        env.plot_v_values(
            agent.Q_network,
            ax=ax,
        ...
        if i < numAx:
            ...
            if instantList is not None:
                if showCapture:
                    env.plot_target_failure_set(
                        ax=ax,
                        xPursuer=traj_x[instant],
                        yPursuer=traj_y[instant],
                        lw=lw,
                    )
                    ax.scatter(
                        traj_x[instant],
            ...
            env.plot_formatting(ax=ax)
            ax.set_title(
                "[{:d}]: {:.3f}".format(testIdx, maxminV), fontsize=12
            )
            ax.set_xticklabels([])
            ax.set_yticklabels([])
    plt.tight_layout()
...
def pursuerResponse(env, agent, statePursuer, trajEvader):
    ...
    for t in range(trajEvader.shape[0]):
        ...
        if not donePursuer:
            ...
            statePursuer = env.pursuer.integrate_forward(
