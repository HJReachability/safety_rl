...
class DubinsCarPEEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # cost
        assert self.mode == 'RA', (
            "PE environment doesn't support conventional RL yet"
        )
        if self.mode == 'RA':
            if fail:
                cost = self.penalty
    ...
    def set_capture_range(self, capture_range=.1):
        ...
        Args:
            capture_range (float, optional): set the capture radius of the
                pursuer. Defaults to .1.
        """
    ...
    def set_constraint(
        self, center=np.array([0., 0.]), radius=1., car='evader'
    ):
        """
    ...
    def get_value(
        ...
        get_value: get the state values given the Q-network. We fix evader's
            heading angle to theta and pursuer's state to
    ...
    def simulate_trajectories(
        ...
        Returns:
            list of np.ndarray: each element is a tuple consisting of
                trajectories of the evader and pursuer.
    ...
    def visualize(
        ...
        Args:
            ...
            theta (float, optional): if provided, set the theta to its value.
                Defaults to np.pi/2.
            rndTraj (bool, optional): random trajectories or not.
                Defaults toFalse.
            num_rnd_traj (int, optional): #states. Defaults to None.
            keepOutOf (bool, optional): smaple states inside the obstacles or
                not. Defaults to False.
        ...
        for i, (ax, state) in enumerate(zip(axes, self.visual_initial_states)):
            ...
            # == Plot V ==
            self.plot_v_values(
                q_func, ax=ax, fig=fig, theta=state[0][2],
                xPursuer=state[0][3], yPursuer=state[0][4],
                thetaPursuer=state[0][5], vmin=vmin, vmax=vmax, nx=nx, ny=ny,
                cmap=cmap, boolPlot=boolPlot, cbarPlot=cbarPlot,
                addBias=addBias
            )
            ...
            if rndTraj:
                self.plot_trajectories(
                    q_func, T=200, num_rnd_traj=num_rnd_traj, theta=theta,
                    toEnd=False, keepOutOf=keepOutOf, ax=ax, orientation=0
                )
            else:
                self.plot_trajectories(
                    q_func, T=200, states=state, toEnd=False, ax=ax,
                    orientation=0
                )
    ...
    #  2D-plot based on evader's x and y
    def plot_v_values(
        self, q_func, theta=0, xPursuer=.5, yPursuer=.5, thetaPursuer=0,
        ax=None, fig=None, vmin=-1, vmax=1, nx=101, ny=101, cmap='seismic',
        ...
        Args:
            q_func (object): agent's Q-network.
            theta (float, optional): if provided, fix the evader's heading
                angle to its value. Defaults to 0.
            xPursuer (float, optional): if provided, fix the pursuer's x
                position to its value. Defaults to .5.
            yPursuer (float, optional): if provided, fix the pursuer's y
                position to its value. Defaults to .5.
            thetaPursuer (int, optional): if provided, fix the pursuer's
                heading angle to its value. Defaults to 0.
            ...
            boolPlot (bool, optional): plot the values in binary form.
                Defaults to False.
            cbarPlot (bool, optional): plot the color bar or not.
                Defaults to True.
            addBias (bool, optional): adding bias to the values or not.
                Defaults to False.
        """
        ...
        if boolPlot:
            im = ax.imshow(
                v.T > 0., interpolation='none', extent=axStyle[0],
