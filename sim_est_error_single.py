...
def run(args):
    ...
    if args.plotFigure or args.storeFigure:
        ...
        ax = axes[0]
        im = ax.imshow(carOneDict['ddqnValue'][:,:,0].T,
        ...
        #== Rollout ==
        ax = axes[1]
        im = ax.imshow(carOneDict['rolloutValue'][:,:,0].T <= 0,
                    interpolation='none', extent=axStyle, origin="lower",
...
if __name__ == '__main__':
    ...
    parser.add_argument("-low", "--low",            help="lowOmega",
        action="store_true")
    
    # Simulation Parameters
    parser.add_argument("-te",  "--toEnd",          help="stop until boundary",
        action="store_true")
