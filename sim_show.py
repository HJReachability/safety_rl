...
parser.add_argument(
    "-t", "--thickness", help="thickness of the obstacle",
    default=0.1, type=float
)
parser.add_argument(
    "-r", "--reward", help="when entering target set", default=-1, type=float)
parser.add_argument(
    "-p", "--penalty", help="when entering failure set",
...
if args.easy:
    outFolder = os.path.join(
        args.outFolder, 'show-easy', 'DDQN', args.mode, fn)
else:
...
env = gym.make(
    ...
    thickness=args.thickness, sample_inside_obs=sample_inside_obs,
    envType=envType
)
...
print(
    "Cost type: {}, Margin scaling: {:.1f}, ".format(env.costType, env.scaling)
...
agent = DDQNSingle(
    CONFIG, actionNum, action_list, dimList=dimList,
    mode=agentMode, terminalType=args.terminalType
)
